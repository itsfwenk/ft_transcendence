...
export async function fetchUserProfile() {
	try {
		...
		if (!response.ok) {
			throw new Error(
				`${i18n.t("profile.errorFetchingProfile")}: ${
					response.statusText
				}`
			);
		}
	...
	if (app) {
		...
		<div class="flex justify-center items-center gap-9">
			...
			border-b-[1px] border-red-400'>
				<span class='flex flex-col justify-center items-center h-full text-white font-jaro text-5xl '>${i18n.t(
					"gameMode.1v1Local"
				)}</span>
			</div>
		...
		const playLocalButton = document.getElementById(
			"localBtn"
		) as HTMLButtonElement; // Changed to HTMLButtonElement
		...
		const playOnlineButton = document.getElementById(
			"onlineBtn"
		) as HTMLFormElement;
		playOnlineButton.addEventListener("click", async (e) => {
			...
			console.log(
				`${i18n.t("profile.currentPlayerId")}:`,
				currentPlayerId
			);
		...
		const playTournamentButton = document.getElementById(
			"tournamentBtn"
		) as HTMLFormElement;
