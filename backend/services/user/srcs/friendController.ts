...
export async function addFriendController(
	req: FriendRequest,
	reply: FastifyReply
) {
	try {
...
export async function removeFriendController(
	...
	try {
		...
		if (!isFriend) {
			return reply
				.status(404)
				.send({ error: "This user is not in your friends list" });
		}
...
export async function getOnlineFriendsController(
	req: AuthenticatedRequest,
	reply: FastifyReply
) {
	try {
...
export async function checkFriendshipController(
	req: FriendIdRequest,
	reply: FastifyReply
) {
	try {
