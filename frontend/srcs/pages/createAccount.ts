	...
	if (app) {
	  app.innerHTML = /*html*/`
		...
		<form id="registerForm" class="flex flex-col gap-6 justify-center items-center">
			...
			<button type="submit" class="font-inria mt-8 w-1/4 border border-black rounded-md font-medium text-white bg-blue-600 hover:bg-blue-700 hover:outline-none focus:outline-none">${i18n.t('login.createAccount')}</button>
		</form>
		<div id="registerStatus" class="mt-4"></div>
        <div class="mt-6">
          <a href="/" data-link class="text-blue-600 hover:underline">${i18n.t('backLog')}</a>
        </div>
	  `;
	...
	registerForm?.addEventListener('submit', async (e) => {
	  ...
	  try {
		...
		registerStatus.innerHTML = /*html*/ `
		  <p class="text-green-500">${i18n.t('logTrue')}</p>
		`;
