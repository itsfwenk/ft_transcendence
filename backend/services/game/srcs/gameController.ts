...
const ballInitialSpeed: number =
	parseInt(process.env.BALL_SPEED as string, 10) || 5;
...
export async function startGame(
	req: FastifyRequest<{
		Body: { player1_id: string; player2_id: string; matchId?: string };
	}>,
	reply: FastifyReply
) {
...
async function getUserById(userId: string) {
	try {
		...
		if (axios.isAxiosError(error)) {
			console.error(
				`Erreur Axios lors de la récupération de l'utilisateur ${userId}:`,
				error.response?.data || error.message
			);
		} else if (error instanceof Error) {
			console.error(
				`Erreur générique lors de la récupération de l'utilisateur ${userId}:`,
				error.message
			);
		} else {
...
async function updateUserGameId(userId: string, gameId: string) {
	try {
		...
		if (axios.isAxiosError(error)) {
			console.error(
				`❌ Axios error while updating user ${userId}:`,
				error.response?.data || error.message
			);
		} else if (error instanceof Error) {
			console.error(
				`❌ Generic error while updating user ${userId}:`,
				error.message
			);
		} else {
	...
	try {
		...
		if (game.leftPaddle && game.rightPaddle) {
			...
			game.leftPaddle.y = Math.max(
				0,
				Math.min(game.canvasHeight - paddleHeight, game.leftPaddle.y)
			);
			game.rightPaddle.y = Math.max(
				0,
				Math.min(game.canvasHeight - paddleHeight, game.rightPaddle.y)
			);
		}
	} catch (err) {
...
async function markPlayerAsForfeit(quitterId: string): Promise<void> {
	...
	const winnerId =
		game.player1_id === quitterId ? game.player2_id : game.player1_id;
...
export async function updatePaddleDeltaInOngoingGames(
	gameId: string,
	playerId: string,
	delta: number
) {
...
export async function websocketHandshake(
	...
	const { value: unsignedToken, valid } = req.unsignCookie(
		req.cookies["authToken"] as string
	);
	console.log(`token:`, unsignedToken);
	if (!unsignedToken) return;
	...
	endedAt: new Date(), activeUsers.set(userId, connection);
		...
		try {
			...
			if (type === "ready_to_start") {
				...
				if (
					...
					console.log(
						`Both players ready for game ${gameId}. Starting game.`
					);
					game.status = "ongoing";
				...
				const delta =
					key === "ArrowUp"
						...
						? paddleSpeed
						: 0;
				updatePaddleDeltaInOngoingGames(
					gameId,
					userId,
					state === "keydown" ? delta : 0
				);
		...
		if (readyPlayers?.size === 0) {
			...
			try {
				const baseUrl =
					...
					"http://matchmaking:4003";
				const response = await axios.post(
					`${baseUrl}/matchmaking/match/update/${updatedGame.matchId}`,
					{
				...
				console.error(
					"Erreur lors de la mise à jour du matchmaking:",
					error
				);
			}
...
export function endOngoingGamesForfeit(
	req: FastifyRequest<{
