...
const dbPath = path.resolve("/app/db/games.db");
setInterval(() => {
	try {
		const { size } = fs.statSync(dbPath);
		sqliteDbFileBytes.set({ service: "game" }, size);
	} catch {
		sqliteDbFileBytes.set({ service: "game" }, 0);
	}
}, 30000);

export function instrumentedRun<T>(
	service: string,
	...
	const end = sqliteQueryDuration.startTimer({
		service,
		statement: statementName,
	});
	try {
		const result = fn();
		sqliteQueriesTotal.inc({
			service,
			status: "ok",
			statement: statementName,
		});
