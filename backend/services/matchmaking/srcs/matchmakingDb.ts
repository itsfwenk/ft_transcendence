...
export function getTournamentById(tournament_Id: string): Tournament | null {
	const row = instrumentedRun(
		"matchmaking",
		"SELECT Tournament by id",
		() =>
			db
				.prepare("SELECT * FROM Tournament WHERE id = ?")
				.get(tournament_Id) as TournamentRow | undefined
	);
	if (!row) return null;

	const matches = instrumentedRun(
		"matchmaking",
		"SELECT Match by tournament_Id",
		() =>
			db
				.prepare("SELECT * FROM Match WHERE tournament_Id = ?")
				.all(tournament_Id) as MatchRow[]
	);
...
export function getMatchHistoryByUserId(userId: string) {
	const matches = instrumentedRun(
		"matchmaking",
		"SELECT Match history by userId",
		() => {
			const stmt = db.prepare(`
