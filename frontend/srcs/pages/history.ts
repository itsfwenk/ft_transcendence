...
export default function History() {
  ...
  if (app) {
	...
	<div class="mx-auto w-4/5 bg-yellow-500 rounded-lg p-6 mb-10">
		<h1 class="text-4xl font-bold mb-6 text-center font-jaro">${i18n.t('profile.matchHistory')}</h1>

		<div class="max-h-80 overflow-y-auto overflow-x-auto rounded-lg">
			<table class="w-full bg-black rounded-lg">
				<thead class="sticky top-0">
					<tr class="bg-gray-50">
						<th class="text-black font-jaro py-3 px-6 font-bold select-none text-center">${i18n.t('history.type')}</th>
						<th class="text-black font-jaro py-3 px-6 font-bold select-none text-center">${i18n.t('history.opponent')}</th>
						<th class="text-black font-jaro py-3 px-6 font-bold select-none text-center">${i18n.t('game.score')}</th>
						<th class="text-black font-jaro py-3 px-6 font-bold select-none text-center">${i18n.t('history.date')}</th>
					</tr>
				</thead>
				<tbody id="matchHistoryBody" class="bg-gray-600">
					<tr>
						<td colspan="4" class="py-3 px-6 text-center text-white">${i18n.t('history.loading')}</td>
					</tr>
				</tbody>
			</table>
		</div>
