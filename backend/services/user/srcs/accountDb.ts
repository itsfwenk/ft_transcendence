...
export function linkAccount(
	...
	try {
		...
		if (existingAccount) { // refresh ses tokens si existe deja
			...
			updateStmt.run(accessToken, refreshToken, tokenExpiry, userId, provider);
		} else { // creer une nouvelle liaison 
			const insertStmt = db.prepare(`
				INSERT INTO linked_accounts (userId, provider, providerId, accessToken, refreshToken, tokenExpiry)
				VALUES (?, ?, ?, ?, ?, ?)
			`);

			insertStmt.run(userId, provider, providerId, accessToken, refreshToken, tokenExpiry);
		}
		return true;
	} catch (error) {
...
export function getLinkedAccount(userId: string, provider: string): LinkedAccount | undefined {
	try {
...
export function getAllLinkedAccounts(userId: string): LinkedAccount[] {
	try {
		const stmt = db.prepare(`SELECT * FROM linked_accounts WHERE userId = ?`);
...
export function unlinkAccount(userId: string, provider: string): boolean {
	try {
		const stmt = db.prepare(`DELETE FROM linked_accounts WHERE userId = ? AND provider = ?`);
...
export function findUserByProviderId(provider: string, providerId: string): string | null {
	try {
