...
export default function createAccount() {
	...
	if (app) {
		app.innerHTML = /*html*/ `
		<div class="text-black font-jaro text-9xl mt-16 mb-28 select-none">${i18n.t(
			"general.pongGame"
		)}</div>
		<form id="registerForm" class="flex flex-col gap-6 justify-center items-center">
        
			<input id="username" type="username" id="username" name="username" placeholder="${i18n.t(
				"login.username"
			)}" required class="shadow-md h-18 pl-6 w-100 bg-white border border-black font-jaro text-black text-2xl rounded-md focus:outline-none">

			<input id="email" type="email" id="email" name="email" placeholder="${i18n.t(
				"login.email"
			)}" required class="shadow-md h-18 pl-6 w-100 bg-white border border-black font-jaro text-black text-2xl rounded-md focus:outline-none">

			<input id="password" type="password" id="password" name="password" placeholder="${i18n.t(
				"login.password"
			)}" required class="shadow-md h-18 pl-6 w-100 bg-white border border-black font-jaro text-black text-2xl rounded-md focus:outline-none">
		...
		<div class="mt-12 flex justify-center">
			<div id="backToLoginBtn" class='button w-32 h-10 bg-gray-700 rounded-full cursor-pointer select-none
				...
				<span class='flex flex-col justify-center items-center h-full text-white font-jaro text-base'>${i18n.t(
					"login.backLog"
				)}</span>
			</div>
		</div>
...
async function handleRegistrationSubmit(e?: Event) {
	...
	try {
		...
		if (!response.ok) {
			registerStatus.innerHTML = `
                <p class="text-red-500 font-jaro">${i18n.t(
					"login.logFalse"
				)}</p>
            `;
			registerStatus.classList.remove("invisible", "opacity-0");
