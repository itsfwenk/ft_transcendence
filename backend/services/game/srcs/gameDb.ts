...
db.exec(`
	...
	CREATE TABLE IF NOT EXISTS games (
	  ...
	  rightPaddle TEXT NOT NULL DEFAULT '{"x": ${
			canvasWidth - 10
		}, "y": ${paddleBasePosition}, "dy": 0}',
	  ball TEXT NOT NULL DEFAULT '{"x": ${canvasWidth / 2}, "y": ${
	canvasHeight / 2
}, "radius": ${ballRadius}, "dx": ${Math.random() > 0.5 ? 3 : -3}, "dy": ${
	Math.random() > 0.5 ? 3 : -3
}}',
	...
	try {
		...
		if (leftPaddle && rightPaddle) {
			...
			leftPaddle.y = Math.max(
				0,
				Math.min(game.canvasHeight - paddleHeight, leftPaddle.y)
			);
			rightPaddle.y = Math.max(
				0,
				Math.min(game.canvasHeight - paddleHeight, rightPaddle.y)
			);
...
export function updatePaddleDelta(
	gameId: string,
	playerId: string,
	delta: number
) {
...
export function saveGameInDb(game: Game) {
	return instrumentedRun("game", "INSERT Game", () => {
		try {
			const stmt = db.prepare(`
				INSERT INTO games (
