...
export async function fetchUserAvatar(userId: string): Promise<string> {
	try {
		...
		if (!response.ok) {
			console.warn(
				`${i18n.t("queue.errorFetchingAvatar")} (${response.status}): ${
					response.statusText
				}`
			);
			return "";
...
export default async function Queue() {
  ...
  `;
	...
	function updateOpponentDisplay(player: {
		userId: string;
		userName: string;
	}) {
		...
		if (container) {
			...
			container.innerHTML = renderPlayerBox(
				player.userId,
				player.userName || i18n.t("queue.opponent"),
				avatarUrl
			);
		}
	...
	function startCountdown1v1(
		...
		if (statusMessage) {
			statusMessage.textContent =
				i18n.t("queue.gameStartingIn") + timeLeft;
		}
			...
			switch (msg.type) {
				case "QUEUE_1V1_PLAYER_JOINED":
					...
					updateOpponentDisplay({
						userId,
						userName: userName || i18n.t("queue.opponent"),
					});
					break;
				...
				case "MATCH_PREP":
					...
					const delay = Number(
						import.meta.env.VITE_1V1_LAUNCH_DELAY ?? "5"
					);
					if (currentGameId)
				...
				case "OPPONENT_FORFEIT":
					...
					history.pushState(
						null,
						"",
						`/game?gameSessionId=${msg.payload.gameSessionId}`
					);
					window.dispatchEvent(new PopStateEvent("popstate"));
			}
		} catch (error) {
