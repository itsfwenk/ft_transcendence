...
function showNotification(
	...
	if (!notificationElement) {
		...
		notificationElement.className =
			"fixed bottom-4 right-4 px-4 py-2 rounded-md shadow-lg z-20 flex items-center";
    ...
    <div>
      <div id="twoBox" class="flex justify-center items-center mb-10 gap-5">
        <div id="profilBox" class="h-65 w-1/3 bg-blue-700 rounded-lg p-4 text-white">
          <div id="img_name" class="flex items-center mb-4">
            ...
            <div class="flex flex-col">
              ...
              <div id="usernameEditContainer" class="hidden flex items-center mt-1">
                <input type="text" id="usernameInput" maxlength="7" class="bg-white text-black rounded-md w-1/2 px-2 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400" placeholder="${i18n.t(
					"editProfile.maxCharsUsername"
				)}" />
          ...
          <div id="emailContainer" class="flex items-center mb-3">
            <div id="email" class="flex font-jaro select-none">${i18n.t(
				"general.loading"
			)}</div>
          ...
          <div id="emailEditContainer" class="hidden flex items-center mb-3">
            <input type="email" id="emailInput" class="bg-white text-black rounded-md px-2 py-1 text-sm w-full mr-2 focus:outline-none focus:ring-2 focus:ring-blue-400" placeholder="${i18n.t(
      ...
      <div id="passwordChangeContainer" class="hidden fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
        <div class="bg-white rounded-lg p-6 shadow-xl w-96 text-black">
          <h3 class="text-3xl font-bold mb-4 font-jaro">${i18n.t(
				"editProfile.changePassword"
			)}</h3>
          
          <div class="mb-4">
            <label for="oldPassword" class="block text-2xl font-medium font-jaro text-gray-700 mb-1">${i18n.t(
				"editProfile.currentPassword"
			)}</label>
            <input type="password" id="oldPassword" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 font-jaro font-light" placeholder="${i18n.t(
				"editProfile.enterCurrentPassword"
			)}" />
          </div>
          
          <div class="mb-4">
            <label for="newPassword" class="block text-2xl font-medium font-jaro text-gray-700 mb-1">${i18n.t(
				"editProfile.newPassword"
			)}</label>
            <input type="password" id="newPassword" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 font-jaro font-light" placeholder="${i18n.t(
				"editProfile.enterNewPassword"
			)}" />
            <p class="text-sm text-gray-500 mt-1 font-jaro font-light">${i18n.t(
				"editProfile.passwordRequirement"
			)}</p>
          </div>
          
          <div class="mb-4">
            <label for="confirmPassword" class="block text-2xl font-medium font-jaro text-gray-700 mb-1">${i18n.t(
				"editProfile.confirmPassword"
			)}</label>
            <input type="password" id="confirmPassword" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 font-jaro font-light" placeholder="${i18n.t(
				"editProfile.confirmNewPassword"
			)}" />
          </div>
...
async function changePassword(
	...
	try {
		...
		if (!response.ok) {
			const data = await response.json();
			throw new Error(
				data.error || i18n.t("editProfile.errorChangingPassword")
			);
		}
...
function showUsernameEdit() {
	...
	const usernameEditContainer = document.getElementById(
...
function updateUIForGoogleUser(isGoogle: boolean) {
	...
	if (changePasswordBtn) {
		if (isGoogle) {
			...
			changePasswordBtn.setAttribute(
				"title",
				i18n.t("editProfile.notAvailableForGoogle")
			);

			changePasswordBtn.addEventListener("mouseenter", () => {
				showNotification(
					i18n.t("editProfile.googlePasswordRestriction"),
					false
				);
			});
		}
...
async function setupEditProfilePage() {
	...
	const cancelPasswordChangeBtn = document.getElementById(
	...
	if (savePasswordChangeBtn) {
		savePasswordChangeBtn.addEventListener("click", async () => {
			...
			try {
				...
				savePasswordChangeBtn.textContent = i18n.t(
					"editProfile.processing"
				);
				...
				if (success) {
					showNotification(
						i18n.t("editProfile.passwordChangedSuccess"),
						true
					);
					togglePasswordChange(false);
				} else {
				...
				showError(
					error.message || i18n.t("editProfile.errorChangingPassword")
				);
			} finally {
			...
			try {
				if (changedData.avatarFile) {
					const avatarSuccess = await uploadAvatar(
						changedData.avatarFile
					);
					if (!avatarSuccess) {
						success = false;
						errorMessage +=
							i18n.t("editProfile.errorUploadAvatar") + " ";
					}
				}

				if (
					changedData.userName ||
					(changedData.email && !isGoogleUser)
				) {
					const updateData: { userName?: string; email?: string } =
						{};
					if (changedData.userName)
						updateData.userName = changedData.userName;
					if (changedData.email && !isGoogleUser)
						updateData.email = changedData.email;

					const profileSuccess = await updateUserProfile(updateData);
					if (!profileSuccess) {
						success = false;
						errorMessage +=
							i18n.t("editProfile.errorUpdateProfile") + " ";
					}
				}

				if (success) {
					showNotification(
						i18n.t("editProfile.profileUpdatedSuccess"),
						true
					);
					...
					showError(
						`${i18n.t(
							"editProfile.updateFailed"
						)}: ${errorMessage}${i18n.t(
							"editProfile.checkConnection"
						)}`
					);
				}
			} catch (error) {
