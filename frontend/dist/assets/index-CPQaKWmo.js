(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))l(a);new MutationObserver(a=>{for(const r of a)if(r.type==="childList")for(const i of r.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&l(i)}).observe(document,{childList:!0,subtree:!0});function o(a){const r={};return a.integrity&&(r.integrity=a.integrity),a.referrerPolicy&&(r.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?r.credentials="include":a.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function l(a){if(a.ep)return;a.ep=!0;const r=o(a);fetch(a.href,r)}})();console.log("[Tournament] Module chargé");function k(){const t=document.getElementById("app");t&&(t.innerHTML=`
      ...
      `,p!==null&&(console.log("currentState",p),E(p)))}function E(t){const e=document.getElementById("app");if(e)switch(t){case"tournament_queue":e.innerHTML="<h2>En attente de joueurs...</h2>";break;case"tournament_launch":e.innerHTML=`
		...
		`,document.getElementById("localBtn").addEventListener("click",a=>{a.preventDefault(),console.log("local button clicked, navigating to /local"),console.log("before history pushState"),history.pushState(null,"","/local"),window.dispatchEvent(new PopStateEvent("popstate"))}),document.getElementById("onlineBtn").addEventListener("click",async a=>{a.preventDefault(),console.log("online button...");const r=await m();if(console.log(r),!r){console.error("Aucun utilisateur connecté");return}const i=r.userId;console.log("currentPlayerId:",i);const s=x();if(!s||s.readyState!==WebSocket.OPEN){console.error("Socket non connectée");return}s.send(JSON.stringify({action:"join_1v1",payload:{}})),history.pushState(null,"","/queue"),window.dispatchEvent(new PopStateEvent("popstate"))}),document.getElementById("tournamentBtn").addEventListener("click",async a=>{a.preventDefault(),console.log("tournament button...");const r=await m();if(console.log(r),!r){console.error("Aucun utilisateur connecté");return}const i=r.userId;console.log("currentPlayerId:",i);const s=x();if(!s||s.readyState!==WebSocket.OPEN){console.error("Socket non connectée");return}s.send(JSON.stringify({action:"join_tournament",payload:{}})),history.pushState(null,"","/queue_tournament"),window.dispatchEvent(new PopStateEvent("popstate"))}))}function T(){const t=document.getElementById("app");console.log("Module Home recharge"),t&&(t.innerHTML=`
        <div class="flex flex-col items-center justify-center min-h-screen">
          <h1 class="text-3xl font-bold text-blue-600 mb-4">Bienvenue sur Pong Game</h1>
          <form id="loginForm" class="space-y-4">
      ...
      `,document.getElementById("loginForm").addEventListener("submit",async o=>{o.preventDefault();const l=document.getElementById("email").value,a=document.getElementById("password").value;try{const r=window.location.origin,i=await fetch(`${r}/user/login`,{method:"POST",credentials:"include",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:l,password:a})});if(!i.ok)throw new Error(`Erreur lors de la connexion: ${i.statusText}`);const s=await i.json();console.log("Réponse de login:",s);const c=await m();c&&c.userId?L(c.userId):console.error("Impossible de recuperer le profile du user"),history.pushState(null,"","/menu"),window.dispatchEvent(new PopStateEvent("popstate"))}catch(r){console.error("Erreur de login:",r)}}))}function B(){const t=document.getElementById("app");t&&(t.innerHTML=`
        <div class="p-4">
	...
	`;const e=await m();if(console.log(e),!e){console.error("Aucun utilisateur connecté");return}const o=e.userId;console.log("currentPlayerId:",o)}function F(){let t=document.getElementById("gameCanvas"),e=t.getContext("2d");if(!e)throw new Error("Failed to get 2D context");const o=10,l=80,a=5;let r=0,i=0,s={x:0,y:t.height/2-l/2,dy:0},c={x:t.width-10,y:t.height/2-l/2,dy:0},n={x:t.width/2,y:t.height/2,radius:7,dx:Math.random()>.5?3:-3,dy:Math.random()>.5?3:-3};document.addEventListener("keydown",u=>{u.key==="w"&&(s.dy=-5),u.key==="s"&&(s.dy=a),u.key==="ArrowUp"&&(c.dy=-5),u.key==="ArrowDown"&&(c.dy=a)}),document.addEventListener("keyup",u=>{(u.key==="w"||u.key==="s")&&(s.dy=0),(u.key==="ArrowUp"||u.key==="ArrowDown")&&(c.dy=0)});function y(){s.y+=s.dy,c.y+=c.dy,s.y=Math.max(0,Math.min(t.height-l,s.y)),c.y=Math.max(0,Math.min(t.height-l,c.y)),n.y-n.radius<=0?(n.dy*=-1,n.y+=3):n.y+n.radius>=t.height&&(n.dy*=-1,n.y-=3);const u=1.1;if(n.x-n.radius<=s.x+o&&n.y>=s.y&&n.y<=s.y+l){let f=(n.y-(s.y+l/2))/(l/2);n.dx*=-1,n.dy=f*Math.abs(n.dx),n.x+=2,n.dx*=u,n.dy*=u}if(n.x+n.radius>=c.x&&n.y>=c.y&&n.y<=c.y+l){let f=(n.y-(c.y+l/2))/(l/2);n.dx*=-1,n.dy=f*Math.abs(n.dx),n.x-=2,n.dx*=u,n.dy*=u}n.x+=n.dx,n.y+=n.dy,n.x<0?(i++,b()):n.x>t.width&&(r++,b());function b(){n.x=t.width/2,n.y=t.height/2,n.dx=Math.random()>.5?3:-3,n.dy=Math.random()>.5?3:-3}}function h(){e&&(e.clearRect(0,0,t.width,t.height),e.fillStyle="blue",e.fillRect(s.x,s.y,o,l),e.fillStyle="red",e.fillRect(c.x,c.y,o,l),e.beginPath(),e.arc(n.x,n.y,n.radius,0,Math.PI*2),e.fillStyle="black",e.fill(),e.closePath(),e.fillStyle="black",e.font="20px Arial",e.textAlign="center",e.textBaseline="top",e.fillText(`${r}`,t.width/4,30),e.fillText(`${i}`,t.width*3/4,30))}function d(){y(),h(),requestAnimationFrame(d)}d()}function O(){var e;const t=document.getElementById("app");t&&(t.innerHTML=`
    ...
    `,(e=document.getElementById("pauseLocalBtn"))==null||e.addEventListener("click",()=>{alert("Game paused. Click OK to resume.")}),setTimeout(()=>{F()},0))}const A={"/":T,"/profile":B,"/menu":M,"/game":_,"/local":O,"/queue":H,"/queue_tournament":j,"/tournament":k};function $(){function t(){const e=window.location.pathname,o=A[e],l=document.getElementById("app");l&&o?(l.innerHTML="",o()):l.innerHTML="<h1>Page not found</h1>"}document.addEventListener("click",e=>{const o=e.target;if(o.tagName==="A"&&o.href&&o.getAttribute("data-link")!==null){e.preventDefault();const l=new URL(o.href);history.pushState(null,"",l.pathname),t()}}),window.addEventListener("popstate",t),t()}$();
